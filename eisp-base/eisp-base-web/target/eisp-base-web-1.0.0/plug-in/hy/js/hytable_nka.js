	...
	function changeWidth(dom){
		...
		var textWidth = $('#'+$(dom).attr("id")+'_pre').css("width").replace("px","");
...
//			});
	...
	function addModel(para) {
		...
		if (type==""){
			...
			obj_body.find("tr").eq('0').find("*[name]").each(function(){
				if ($(this).attr("name") !="pbNum" && $(this).attr("name") != "id" && unfill.indexOf(","+$(this).attr("name")+",") == "-1") {
	...
	function copyModel(para) {
		...
		$("#"+tableModelArr[num]['tableId']+" tbody").find("tr").eq('0').find("*[type='checkbox']").eq(0).trigger("change");
		return true;
	}
	...
	function searchModel(para){
		...
		var size = para["size"] = (typeof(para["size"])=="undefined"||para["size"]==null)?100:para["size"];
		...
		$.ajax({url:url,data:datastr,type:"post",success:function(bk_json){
	...
	function searchModels(para){
		...
		$.ajax({url:url,data:datastr,type:"post",success:function(bk_json){
			...
			if (bk_arr["success"]){
				if(tipSuccess=="undefined") /*tip("查询成功"); */{} else if(tipSuccess!="") tip(tipSuccess);
				var catalogOne_arr = bk_arr;
				var catalogOne ='obj,xpsPriceGuideList';
				var catalogOnes = catalogOne = typeof(catalogOne)=="undefined"?"obj":catalogOne;
				for (var i=0;i<catalogOnes.split(",").length;i++){
					if (catalogOnes.split(",")[i]!="")
						catalogOne_arr = catalogOne_arr[catalogOnes.split(",")[i]];
				}
				//填充行数据
				fillDataModel({
					tagNum:1,
					dataList:catalogOne_arr,
					checked:checked,
					unfill:unfill,
					dataSearch:dataSearch,
					dataBack:dataBack,
				...
				var catalogTwo_arr = bk_arr;
				var catalogTwo ='obj,xpsCostGuideList';
				var catalogTwos = catalogTwo = typeof(catalogTwo)=="undefined"?"obj":catalogTwo;
				for (var i=0;i<catalogTwos.split(",").length;i++){
					if (catalogTwos.split(",")[i]!="")
						catalogTwo_arr = catalogTwo_arr[catalogTwos.split(",")[i]];
				}
				//填充行数据
				fillDataModel({
					tagNum:2,
					dataList:catalogTwo_arr,
					checked:checked,
					unfill:unfill,
					changestr:changestr,
					dataSearch:dataSearch,
			    ...
			    var datas =bk_arr.obj;
				if(datas.xpsMarketGuideList!=null){
					$("#marketId").val(datas.xpsMarketGuideList.id);
					$("#marketTheme").val(datas.xpsMarketGuideList.marketTheme);
					$("#promoProduct").val(datas.xpsMarketGuideList.promoProduct);
					$("#actRange").val(datas.xpsMarketGuideList.actRange);
					$("#beginDate").val(datas.xpsMarketGuideList.beginDate);
					$("#endDate").val(datas.xpsMarketGuideList.endDate);
					$("#estimatedCost").val(datas.xpsMarketGuideList.estimatedCost);
					$("#targetContext").val(datas.xpsMarketGuideList.targetContext);
					$("#actContext").val(datas.xpsMarketGuideList.actContext);
					$("#actDetailed").val(datas.xpsMarketGuideList.actDetailed); 
					$("#issuedType").val(datas.xpsMarketGuideList.issuedType);
					tabs_cover("block");
...
//		});
				...
				obj.find("tr").each(function(e) {
					...
					if (is_check !== ''){
						...
						obj.find("tr").eq(is_check).find("input").each(function(q) {
							formstr += '<input type="text" name="'+namestr+'['+i+'].'+this.name+'"  value="'+(this.name!="pbNum"&&this.value==idarr[i]&&idarr[i].indexOf("_")!="-1"?"":this.value)+'" '+(this.getAttribute('datatype')?'datatype="'+this.getAttribute('datatype')+'"':'')+' '+(this.getAttribute('errormsg')?'errormsg="'+this.getAttribute('errormsg')+'"':'')+' '+(this.getAttribute('nullmsg')?'nullmsg="'+this.getAttribute('nullmsg')+'"':'')+' >';
						});
					}
				});
			}
	...
	function defineDialog(para){
		...
		if (typeof(para["initFunction"])=="undefined" || para["initFunction"]==""){falg = false; } else var initFunction = para["initFunction"];
		var title = para["title"] = typeof(para["title"])=="undefined"?"":para["title"];
		var width = para["width"] = typeof(para["width"])=="undefined"?"":para["width"];
		var height = para["height"] = typeof(para["height"])=="undefined"?"":para["height"];
		createwindowExt(title, addurl, width, height, {
