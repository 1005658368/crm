...
//            update-begin--Author:Biz  Date:20140429 for：一级菜单右侧有双引号，且在ie下样式错位
//            $(".shortcut").html(data);
            $(".shortcut").html(data.replaceAll("\"", ""));
//            update-end--Author:Biz  Date:20140429 for：一级菜单右侧有双引号，且在ie下样式错位
        }
    });
...
function createFrame(url) {
	var s = '<iframe name="tabiframe" id="tabiframe"  scrolling="no" frameborder="0"  src="'+url+'" style="width:100%;height:99.4%;overflow-y:hidden;"></iframe>';
	return s;
}
...
function tabCloseEven() {
	...
	$('#mm-tabcloseleft').click(function() {
		...
		prevall.each(function(i, n) {
			var t = $('a:eq(0) span', $(n)).text();
//            update-begin--Author:Biz  Date:20140612 for：不关闭首页
            if (t != '首页') {
                $('#maintabs').tabs('close', t);
            }
//            update-end--Author:Biz  Date:20140612 for：不关闭首页
		});
		return false;
	});
