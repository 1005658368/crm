...
draw2d.UserTask.prototype.getMultiInstanceXML=function(){
	...
	var sign = '<multiInstanceLoopCharacteristics isSequential="'+this.isSequential+'" activiti:collection="'+this.collection+'" activiti:elementVariable="'+this.elementVariable+'">';
	
	if(typeof(this.completionCondition) != "undefined"){
		this.completionCondition = this.completionCondition.replace(/\s+/g,"");
		if(this.completionCondition!=null && this.completionCondition!="" && this.completionCondition.length > 0){
			sign += '<completionCondition>'+this.completionCondition+'</completionCondition>';
		}
	}
	
	sign += '</multiInstanceLoopCharacteristics>';
