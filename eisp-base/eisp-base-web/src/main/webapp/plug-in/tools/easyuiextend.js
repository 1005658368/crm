// 部分的脚本期待String支持trim方法. 提供一个默认实现.
...
function deploymentimg(deploymentId, resourceName,title) {
	...
	openwindow('流程部署详细图--'+title, 'activitiController.do?openProcessPic&tag=deployment&deploymentId=' + deploymentId + '&resourceName=' + resourceName,800,1000);
}
...
function progress(busKey, title) {
	...
	openwindow('流程进度--当前节点:' + title, 'workflowFysysController.do?goToProcessDetailTemp&businessKey='+busKey,"",950,500);
 ...
 * @param title
 */
function progresses(busKey, title){
    if (typeof (title) == 'undefined') {
        title = "";
    }
    openwindow('流程进度--当前节点:' + title, 'activitiController.do?openProcessPic&tag=project&businessKey='+ busKey,"",950,500);
}
/**
 * 打开流程办理页面
 */
