...
//全选/全不选,dimType=1表示前缀模糊查询，dimType=2表示后缀模糊查询
function selall(name,area,checked,dimType)
{
	var obj="";
	if(1==dimType){
		obj=$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']");
	}else if(2==dimType){
		obj=$((typeof(area)=="undefined"?"":area)+" *[name$='"+name+"']");
	}else{
		obj=$((typeof(area)=="undefined"?"":area)+" *[name='"+name+"']");
...
function selall2(name,area,checked,dimType)
{
	var obj="";
	if(1==dimType){
		if(checked){
			$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']").attr("checked",true);
			$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']").closest("tr").addClass("activeModel");
		}else{
			$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']").attr("checked",false);
			$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']").closest("tr").removeClass("activeModel");
		}
	}else if(2==dimType){
		if(checked){
			$((typeof(area)=="undefined"?"":area)+" *[name$='"+name+"']").attr("checked",true);
...
//全不选
function unselall(name,area,dimType)
{
	var obj="";
	if(1==dimType){
		obj=$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']");
...
//反选
function inselall(name,area,dimType)
{
	var obj="";
	if(1==dimType){
		obj=$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']");
...
//单选
function selone(name,val,area,dimType)
{
	/*
	 ...
	 */
	  var obj="";
	  if(1==dimType){//dimType=1前缀模糊查询，dimType=2后缀模糊查询
		  obj=$((typeof(area)=="undefined"?"":area)+" *[name^='"+name+"']");
	  }else if(2==dimType){
		  obj=$((typeof(area)=="undefined"?"":area)+" *[name$='"+name+"']");
	  }else{
...
//获取checkbox的值
function getcheckboxvalue(name,attr,area,dimType)
{
	return ck_rd_str(name,"checkbox","get_value",attr,area,dimType);
}

//取值单选复选checkbox,radio
//----------checkbox,radio getvalue/getcount------------------------------------
function  ck_rd_str(frm_name,obj_type,back_type,attr,area,dimType)
{   
	  var obj="";
	  if(1==dimType){//dimType=1前缀模糊查询，dimType=2后缀模糊查询
		  obj=$((typeof(area)=="undefined"?"":area)+" *[name^='"+frm_name+"']");
	  }else if(2==dimType){
		  obj=$((typeof(area)=="undefined"?"":area)+" *[name$='"+frm_name+"']");
	  }else{
		  obj=$((typeof(area)=="undefined"?"":area)+" *[name='"+frm_name+"']");
