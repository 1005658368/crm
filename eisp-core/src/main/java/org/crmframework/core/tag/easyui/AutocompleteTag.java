...
public class AutocompleteTag extends TagSupport {
    ...
    public StringBuffer end() {
        ...
        nsb.append("$(document).ready(function() {")
                        ...
                        .append("featureClass : \"P\",style : \"full\",	maxRows : " + maxRows + ",labelField : \""
                                        + labelField + "\",valueField : \"" + valueField + "\",")
                        .append("searchField : \"" + searchField + "\",entityName : \"" + entityName + "\",className : \"" + className
                                        + "\",trem: get"+name+"Value}");
        ...
        if (StringUtil.isNotEmpty(formatItem)) {
        	...
        	nsb.append(",formatItem:function(row, i, max){return " + name + "FormatItem.call(this,row, i, max);} ");
        }
        nsb.append("}).result(function (event, row, formatted) {");
        if (StringUtil.isNotEmpty(result)) {
            nsb.append(result + ".call(this,row); ");
        }else{
        	nsb.append(name + "CallBack.call(this,row); ");
        }
        
        nsb.append("}); });").append("function get"+name+"Value(){return $(\"#" + name + "\").val();}");
        if(StringUtil.isEmpty(formatItem)){
        	if(searchField.contains(",")){
        		String key = searchField.split(",")[0];
        		String value = searchField.split(",")[1];
        		nsb.append(" function " + name + "FormatItem(data) {return data."+key+" + '-->' + data."+value+";} ");
        	}else{
        		nsb.append(" function " + name + "FormatItem(data) {return data."+searchField+";} ");
        	}
        }
        if(StringUtil.isEmpty(result)){
        	if(searchField.contains(",")){
        		String key = searchField.split(",")[0];
        		String value = searchField.split(",")[1];
        		nsb.append(" function " + name + "CallBack(data){$('#"+valueField+"').val(data."+key+");$('#"+name+"').val(data."+value+");}");
        	}else{
        		nsb.append(" function " + name + "CallBack(data){$('#"+name+"').val(data."+searchField+");}");
        	}
        	
        	
        }
        nsb.append("</script>").append("<input type=\"text\" id=\"" + name + "\" datatype=\"" + datatype + "\" nullmsg=\""
                                        + nullmsg + "\" errormsg=\"" + errormsg + "\"/>");
        if (StringUtil.isNotEmpty(label)) {
            nsb.append(" value=" + label + " readonly=true");
        }
        nsb.append("<input type=\"hidden\" id=\"" + valueField + "\" name=\"" + valueField + "\" datatype=\"*\"/>");
        return nsb;
    }
