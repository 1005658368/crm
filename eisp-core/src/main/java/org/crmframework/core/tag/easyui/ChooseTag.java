...
public class ChooseTag extends TagSupport {
    ...
    protected String beforeFunction;//选择前fun
    protected Boolean confirmButton;//弹出框里面是否有确定按钮
    protected String beforeCall;
    ...
    public StringBuffer end() {
        ...
        if(StringUtil.isNotBlank(beforeFunction)){
            sb.append(" "+beforeFunction+"();");
        }
        sb.append("var targetUrl = '"+url+"';");

        if(StringUtil.isNotEmpty(parameter)){
            String[] arrParam = parameter.split(",");
            for(int i=0; i<arrParam.length; i++){
                String param = arrParam[i];
                sb.append("var id_" + param + " = $('#"+param+"').val();");
                sb.append("if(id_" + param + ".length==0||id_" + param + "==''){");
                sb.append("var tit_" + param + " = $('#"+param+"').attr('title');");
                sb.append("tip(tit_" + param + " + '不能为空');");
                sb.append("return ;");
                sb.append("}else{");
                sb.append("targetUrl += '&"+param+"=' + id_" + param);
                sb.append("}");
            }
        }
        ...
        sb.append("$.dialog({");
        sb.append("content: \'url:\' + targetUrl,");
        ...
        sb.append("button : [ ");
        if(null==confirmButton||confirmButton){
            sb.append("{");
    ...
    private void callback(StringBuffer sb, String methodname) {
        ...
        sb.append("iframe = this.iframe.contentWindow;var check=true;");
        if(StringUtil.isNotBlank(beforeCall)){
            sb.append("var tcheck=iframe."+beforeCall+"();");
            sb.append("if(tcheck==false){check=false}");
        }
        sb.append("if(!check){return false}");
        String[] textnames = null;
        String[] inputTextnames = null;
