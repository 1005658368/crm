...
import org.crmframework.core.vo.ColumnValue;
import org.crmframework.core.vo.DataGridColumn;
import org.crmframework.core.vo.DataGridUrl;
import org.crmframework.core.vo.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
    ...
    private boolean newFlag=false;//新旧选择
    private boolean checkbox = false;// 是否显示复选框
    private boolean singleSelect = false;// 是否为单选
     ...
     * @param width2
     */
    public void setToolbar(String id, String url, String title, String icon, String exp, String onclick, String funname,
    ...
    public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align,
                          boolean sortable, boolean checkbox, String formatter, boolean hidden, String replace,
                          String treefield, boolean image, String imageSize, boolean query, String url, String funname,
                          String arg, String queryMode, String dictionary, boolean frozenColumn, String extend, String style,
                          String downloadName, boolean isAuto, String extendParams, String headcolor, String fieldcolor) {
        ...
        dataGridColumn.setHeadcolor(headcolor);
        dataGridColumn.setFieldcolor(fieldcolor);
    ...
    public StringBuffer datatables() {
        ...
        sb.append("\"oLanguage\" : {" + "\"sLengthMenu\" : \" _MENU_ 条记录\"," + "\"sZeroRecords\" : \"没有检索到数据\","
                ...
                + "\"sFirst\" : \"首页\"," + "\"sPrevious\" : \"前页\", " + "\"sNext\" : \"后页\","
                + "\"sLast\" : \"尾页\"" + "}" + "},"); // 汉化
    ...
    public StringBuffer endNew() {
        ...
        if (autoLoadData)
            ...
            sb.append("url:\'',");
        if (StringUtil.isNotEmpty(rowStyler)) {
...
/*        }*/
        ...
        this.getField(sb);
        sb.append("]],");
        sb.append("onLoadSuccess:function(data){ clickcolor();");
        //dropDnd
        if (StringUtil.isNotBlank(dropDnd)) {//拖动功能初始化
            sb.append("$(this).datagrid('enableDnd');");
        }
        ...
        //dropDnd
        if (StringUtil.isNotBlank(dropDnd)) {//拖动功能初始化
            sb.append("onDrop:function(targetRow,sourceRow,point){ "+dropDnd+"(targetRow,sourceRow)},");
        }
        ...
        sb.append("function get" + name + "Selections(field){" + "var ids = [];" + "var rows = $(\'#" + name + "\')."
                + grid + "(\'getSelections\');" + "for(var i=0;i<rows.length;i++){"
        ...
        if (columnList.size() > 0) {
            ...
            // 生成重置按钮功能js
            //额外增强
            if ("single".equals(getQueryMode())) {
                //初始化---避免转换选项之后查询条件未清空
                initParamsTempFun(sb, grid);
            }

            // update-begin Robin 20140426 for:回车事件
            // 回车事件
            // modified by shuai.cui at 2018/01/17 回车事件回调函数添加name
            //原因：页面多个datagrid时生成多个同名的回车事件回调函数导致前面的datagrid搜索框回车事件回调函数被覆盖
            sb.append("function EnterPress"+name+"(e){");
            ...
            //此处修改框架，原因：若页面上有多个datagrid，就会生成多个searchReset()，导致每个datagrid里的重置按钮都调用最后一个searchReset()，而此时刷新页面的方法又是写死名称的，所以每个重置都重置了最后的表格
            //String func = name.trim() + "search();";
            String func = "eval(name+\"search()\");";
            sb.append(func);
            sb.append("}");

        }
        sb.append("function clickcolor(){" + dealColor() + "}");// 更改表头颜色
        sb.append("</script>");
            ...
            if ("group".equals(getQueryMode())) {
                for (DataGridColumn col : columnList) {
                    if (col.isQuery()) {
                        ...
                        sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: 90px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""
                        ...
                        if ("single".equals(col.getQueryMode())) {
                            if (!StringUtil.isEmpty(col.getReplace())) {
                                ...
                                if (col.getDictionary().contains(",")) {
                                    ...
                                    sb.append("<select name=\"" + col.getField().replaceAll("_", "\\.")
                                            + "\" WIDTH=\"100\" style=\"width: 104px\"> ");
                                    sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}",
                                            MutiLangUtil.getMutiLangInstance().getLang(" ")));
                                    for (Map<String, Object> map : list) {
                                    ...
                                    sb.append("<select name=\"" + col.getField().replaceAll("_", "\\.")
                                            + "\" WIDTH=\"100\" style=\"width: 104px\"> ");
                                    sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}",
                                            MutiLangUtil.getMutiLangInstance().getLang("--请选择--")));
                                ...
                                sb.append("<input onkeypress=\"EnterPress"+name+"(event)\" onkeydown=\"EnterPress"+name+"()\"  type=\"text\" name=\""
                            ...
                            sb.append("<input type=\"text\" name=\"" + col.getField()
                                    + "_begin\"  style=\"width: 94px\" " + extendAttribute(col.getExtend())
                                    + "/>");
        ...
        if ("group".equals(getQueryMode()) && hasQueryColum(columnList)) {// 如果表单是组合查询
            ...
            sb.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\""
                    + name
                    + StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance()
                    .getLang("common.query")));
            ...
            sb.append("<input id=\""
                    ...
                    .getMutiLangInstance().getLang("common.please.input.keyword")) + name
                    + "mm\'\"></input>");
            sb.append("<div id=\"" + name + "mm\" style=\"width:120px\">");
            for (DataGridColumn col : columnList) {
                if (col.isQuery()) {
                    sb.append("<div data-options=\"name:\'" + col.getField().replaceAll("_", "\\.")
                            + "\',iconCls:\'icon-ok\' " + extendAttribute(col.getExtend()) + " \">"
                            + col.getTitle() + "</div>  ");
                }
            }
            sb.append("</div>");
...
/*        if (fitColumns) {
    ...
    private String dealColor() {
        StringBuffer sb = new StringBuffer();
        for (DataGridColumn col : columnList) {
            if (StringUtil.isNotBlank(col.getHeadcolor())) {// 判断表头配置是否为空
                // sb.append("function clickcolor(){$(\"[field='tbMatterCode']\").css(\"background-color\",\"red\");}");//更改sft
                sb.append("$(\"[field='" + col.getField() + "']\").eq(0).css(\"background-color\",\""
                        + col.getHeadcolor() + "\"); ");
...
/*                if (column.getWidth() != null) {
                ...
                if (StringUtil.isNotEmpty(column.getExtendParams())) {
                ...
                if (column.getFormatter() != null) {
                    if(column.getFormatter().contains("%")){
                        sb.append(",formatter:function(value,rec,index){");
                        if(column.getFormatter().equals("/%")){
                            //clare
                            sb.append("if (typeof(value) != 'undefined' && value != ''){ var val = value + '%'; return val;} else {return value;}  }");
                        }else{
                            //younger
                            sb.append("if (typeof(value) != 'undefined'){ var val = value*100; var vv = val.toFixed(2) + '%'; return vv;} else {return value;}  }");
                        }
                    }else if(column.getFormatter().contains(",")){
                        sb.append(",formatter:function(value,rec,index){");
                        sb.append("if (typeof(value) != 'undefined'&&value!=''){ "
                                + " s = parseFloat((value + '').replace(/[^\\d\\.-]/g,'')).toFixed(2)+''; "
                                + " var l = s.split('.')[0].split('').reverse(),r = s.split('.')[1];  "
                                + " t = '';  "
                                + " for(i = 0; i < l.length; i ++ )  "
                                + " {    t += l[i] + ((i + 1) % 3 == 0 && (i + 1) != l.length ? ',' : '');     }  "
                                + " return t.split('').reverse().join('') + '.' + r; } else {return value;}  }");
                    }else if(column.getFormatter().equals("null")){
                        sb.append(",formatter:function(value,rec,index){");
                        sb.append("if (typeof(value) != 'undefined' && value != '' && value == 'null'){return ''; } else {return value;}  }");
                    }else{
                        sb.append(",formatter:function(value,rec,index){");
                ...
                if (columnValueList.size() > 0 && !column.getField().equals("opt")) {
                    ...
                    for (ColumnValue columnValue : columnValueList) {
                        if (columnValue.getName().equals(column.getField())) {
                            ...
                            sb.append("else{");
                            for (int j = 0; j < value.length; j++) {
                                testString += "if(value.trim()=='" + value[j] + "'.trim()){return \'" + text[j] + "\'}";
                            }
    ...
    protected void searchboxFun(StringBuffer sb, String grid) {
        ...
        sb.append("var queryParams=$(\'#" + name + "\').datagrid('options').queryParams;");
        if ("single".equals(getQueryMode())) {//单条件查询
            sb.append("initParamsTemp(queryParams);");//初始化---避免转换选项之后查询条件未清空--这是一种外加增强
        }
    ...
    //初始化但条件查询参数---避免转换选项之后查询条件未清空
    protected void initParamsTempFun(StringBuffer sb, String grid){
        sb.append("function initParamsTemp(queryParams){");
        sb.append("$('#" + name + "mm').find('div[class=\"menu-item\"]').each(function(){");
        sb.append("queryParams[$(this).attr('name')] = ''; ");
        sb.append("});");
        sb.append("}");
    }
...
//        org.eispframework.core.util.LogUtil.info("----getNoAuthOperButton-------" + sb.toString());
        return sb.toString();
    }
    ...
    private String getAutoSpan(String filed, String extend) {
        ...
        nsb.append("$(document).ready(function() {")
                ...
                .append("searchField : \"" + filed + "\",entityName : \"" + getEntityName()
                        + "\",trem: function(){return $(\"#" + getEntityName() + "_" + id
                        + "\").val();}}");
        nsb.append(",parse:function(data){return eispAutoParse.call(this,data);}");
    ...
    public String getDropDnd() {
        return dropDnd;
    }

    public void setDropDnd(String dropDnd) {
        this.dropDnd = dropDnd;
    }
