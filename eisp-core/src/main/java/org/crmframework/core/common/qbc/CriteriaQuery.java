...
import org.crmframework.core.vo.datatable.DataTables;
import org.crmframework.core.vo.datatable.SortDirection;
import org.crmframework.core.vo.datatable.SortInfo;
...
/**
 * 
 * 类描述：CriteriaQuery类是对hibernate QBC查询方法的封装，需要的参数是当前操作的实体类 Biz
 *
...
public class CriteriaQuery implements Serializable {
    ...
    /**
     * 加载条件(条件之间有关联) hql((this_.0 like ? and this_.1 like ?) or this_.2 like ?) 表示法cq.add(cq.or(cq.and(cq, 0, 1), cq,
     * 2))----- hql2:(this_.0 like ? or this_.1 like ?) 表示法:cq.add(cq.or(cq, 0, 1)); 例子：cq.in("TBPrjstatus.code",
     * status); cq.eq("attn", user.getUserName()); cq.isNull("attn"); cq.add(cq.and(cq.or(cq, 1, 2), cq, 0));
     */
