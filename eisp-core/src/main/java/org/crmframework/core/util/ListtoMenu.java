...
public class ListtoMenu {
    ...
    static void iterGet(List<TSFunction> set1, String pid, StringBuffer buffer) {
        ...
        for (TSFunction node : set1) {
            ...
            {
                buffer.append("{\"menuid\":\"" + node.getId() + " \",\"icon\":\"" + node.getTSIcon().getIconClas()
    ...
    public static String getEasyuiMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        ...
        for (TSFunction pFunction : pFunctions) {
            menuString.append("<div  title=\"" + getMutiLang(pFunction.getFunctionName()) + "\" iconCls=\""
                            + pFunction.getTSIcon().getIconClas() + "\">");
            int submenusize = pFunction.getTSFunctions().size();
            if (submenusize == 0) {
            ...
            for (TSFunction function : functions) {
                ...
                if (function.getTSFunction().getId().equals(pFunction.getId())) {
                    ...
                    menuString.append("<li><div onclick=\"addTab(\'" + getMutiLang(function.getFunctionName())
                                    + "\',\'" + function.getFunctionUrl() + "&clickFunctionId=" + function.getId()
                                    + "\',\'" + icon + "\')\"  title=\"" + getMutiLang(function.getFunctionName())
                                    + "\" url=\"" + function.getFunctionUrl() + "\" iconCls=\"" + icon
    ...
    // update-start--Author:Biz Date:20140622 for：左侧树调整：加大宽度、更换节点图标、修改选中颜色
    ...
    public static String getEasyuiMultistageTree(Map<Integer, List<TSFunction>> map, String style) {
        ...
        if ("easyui".equals(style)) {
            for (TSFunction function : list) {
            	 String iscons=function.getTSIcon().getIconClas();
                ...
                menuString.append("<span>").append(getMutiLang(fname)).append("</span>");
                int submenusize = function.getTSFunctions().size();
                if (submenusize == 0) {
                    menuString.append("</li>");
                }
    ...
    public static String newLeftStyle(Map<Integer, List<TSFunction>> map, String style) {
    	if (map == null || map.size() == 0 || !map.containsKey(0)) {
    		return "不具有任何权限,\n请找管理员分配权限";
    	}
    	StringBuffer menuString = new StringBuffer();
    	...
    	if ("easyui".equals(style)) {
    		for (TSFunction function : list) {
    			menuString.append("<li>");
    			String fname=function.getFunctionName();
    			menuString.append("<a>").append("<img src='"+function.getTSIcon().getIconPath()+"'/>&nbsp;")
    			 	.append(getMutiLang(fname)).append("</a><ul>");
    			int submenusize = function.getTSFunctions().size();
    ...
    // update-end--Author:Biz Date:20140622 for：左侧树调整：加大宽度、更换节点图标、修改选中颜色
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (function.getTSFunctions().size() == 0 || !map.containsKey(level + 1)) {
                    ...
                    if(null!=function.getTSIcon()&&org.apache.commons.lang3.StringUtils.isNotBlank(function.getTSIcon().getIconClas())){
                        menuString.append("<li state=\"closed\" iconCls=\"" + function.getTSIcon().getIconClas()
                                      + "\" ><a>" + getMutiLang(function.getFunctionName()) + "</a>");
                    }else{
                        menuString.append("<li state=\"closed\" ><a>" + getMutiLang(function.getFunctionName()) + "</a>");
                    }
    ...
    private static String getLeafOfTree(TSFunction function) {
        ...
        if(function.getFunctionType() == 3) {
        	menuString.append("\"> <a onclick=\"addFunctionDialog(\'");
        } else {
    ...
    public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('"
                            + pFunction.getTSIcon().getIconPath() + "')\"></span> " + pFunction.getFunctionName() + " ");
            if (hasSub) {
    ...
    public static String getWebosMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("{\"id\":\"" + pFunction.getId() + "\",\"name\":\"" + pFunction.getFunctionName()
                            + "\",\"path\":\"" + pFunction.getTSIcon().getIconPath() + "\",\"level\":\""
                            + pFunction.getFunctionLevel() + "\",");
    ...
    private static String getWebosChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                ...
                // }
                menuString.append("\"" + function.getId() + "\":");
                menuString.append("{\"id\":\"" + function.getId() + "\",\"name\":\""
                                + getMutiLang(function.getFunctionName()) + "\",\"path\":\""
                                + function.getTSIcon().getIconPath() + "\",\"url\":\"" + function.getFunctionUrl()
    ...
    // update-begin--Author:Biz Date:20140512 for：添加云桌面图标管理
    ...
    // update-end--Author:Biz Date:20140512 for：添加云桌面图标管理
    // update-start--Author:gaofeng Date:2014-02-14：新增Webos头部菜单导航，多级菜单
}
