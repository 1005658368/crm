package org.crmframework.core.common2.dao.impl;

import freemarker.template.utility.StringUtil;
import org.apache.commons.lang3.StringUtils;
import org.crmframework.core.annotation.CrmEntityTitle;
import org.crmframework.core.common.dao.jdbc.JdbcDao;
import org.crmframework.core.common.exception.BusinessException;
import org.crmframework.core.common.qbc.*;
import org.crmframework.core.common.qbc.CriteriaQuery;
import org.crmframework.core.common2.dao.IGenericBaseCommonDao;
import org.crmframework.core.util.MyBeanUtils;
import org.crmframework.core.util.ToEntityUtil;
import org.crmframework.core.util.oConvertUtils;
import org.crmframework.core.vo.DBTable;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	...
	public List<DBTable> getAllDbTableName() {
		...
		for (String key : (Set<String>) metaMap.keySet()) {
			...
			try {
				c = Class.forName(key);
				CrmEntityTitle t = c.getAnnotation(CrmEntityTitle.class);
...
//		}
	...
	public List<Map<String, Object>> findForJdbc(String sql, int page, int rows) {
		// 封装分页SQL
		sql = JdbcDao.crmCreatePageSql(sql, page, rows);
	...
	public Object executeSqlReturnKey(final String sql, Map<String, Object> param) {
		...
		if (StringUtils.isNotEmpty(String.valueOf(param.get("id")))) {//表示已经生成过id(UUID),则表示是非序列或数据库自增的形式
			this.namedParameterJdbcTemplate.update(sql,sqlp);
		//--author：zhoujf---start------date:20170216--------for:自定义表单保存数据格sqlserver报错问题
		}else if (StringUtils.isNotEmpty(String.valueOf(param.get("ID")))) {//表示已经生成过id(UUID),则表示是非序列或数据库自增的形式
			this.namedParameterJdbcTemplate.update(sql,sqlp);
